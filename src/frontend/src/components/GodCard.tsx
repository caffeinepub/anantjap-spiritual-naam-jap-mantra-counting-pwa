...
export default function GodCard({
  ...
  return (
    <div className="group hover:shadow-lg transition-all duration-300 overflow-hidden rounded-xl border border-border bg-card text-card-foreground shadow-sm">
      ...
      >
        ...
        />
        <div className="absolute inset-0 anant-gradient opacity-0 group-hover:opacity-20 transition-opacity" />
      </button>
            ...
            <div className="flex items-center gap-1 -mt-1 -mr-1 shrink-0">
              {onEdit && (
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-8 w-8 text-muted-foreground hover:text-terracotta"
      ...
      <button
        type="button"
        className="px-4 pb-4 space-y-3 w-full text-left cursor-pointer"
        onClick={onSelect}
